...
#Now calculate life expectancy using life table methods

glimpse(deaths_popns)
unique(deaths_popns$age_grp)
le_data <- deaths_popns %>%
  filter(age_grp !="All")

test <- le_data %>%
  filter(Year == "2017", deprivation_decile == "1", Sex == "Male")
rad <- 100000
test$mx <- test$deaths/test$popn
test$ax <- c(0.3, rep(0.5,18),1/test$mx[20])
test$qx <- test$mx/(1+(1-test$ax)*test$mx)
test$qx[20] <- 1
test$px <- 1-test$qx
test$lx <- c(1*rad, (cumprod(1-test$qx[-20]))*rad)
test$dx <- test$lx*test$qx
test$Lx <- test$lx-(1-test$ax)*test$dx
test$Lx[20] <- test$ax[20]*test$dx[20]
test$Tx <- rev(cumsum(rev(test$Lx)))
test$ex <- test$Tx/test$lx

#Alternative approach using LifeTables package
nmx <- test$mx
testing <- lt.mx(nmx, sex="male", age=c(0,1,seq(5,90,5)), nax=NULL)

#Do this for deciles 1 and 10 for all years and genders
le_dec1_2001_m <- le_data %>%
  filter(deprivation_decile == "1", Year == "2001", Sex == "Male") %>%
  mutate(nmx = deaths/popn) %>%
  select(-Year, -deprivation_decile, -Sex, -age_grp, -deaths, -popn)
le_dec1_2001_m <- lt.mx(nmx=nmx, sex="male", age=c(0,1,seq(5,90,5)), nax=NULL)

le_dec10_2001_m <- le_data %>%
  filter(deprivation_decile == "10", Year == "2001", Sex == "Male") %>%
  mutate(nmx = deaths/popn) %>%
  select(-Year, -deprivation_decile, -Sex, -age_grp, -deaths, -popn)
le_dec10_2001_m <- lt.mx(nmx=nmx, sex="male", age=c(0,1,seq(5,90,5)), nax=NULL)
...
life_tab_m_2017_dec1 <- test %>%
  filter(Sex == "M" & deprivation_decile == "1" & Year == "2017") %>%
  lt.mx(nmx = age_spec_rate, sex = "male", age = c(0,1,seq(5,90,5)), nax=)
